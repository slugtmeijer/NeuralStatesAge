...
from plot_time_correlations_revis import plot_time_correlation_boundaries
...
def simulate_noise(group, SL, rep, noiseperc, noiseBOLD, noiseBOLD_name, peak_delay, peak_disp, TR, loaddir, savedir):
    ...
    for nb in noiseBOLD:
        ...
        if rep == 0:
            ...
            plot_time_correlation_boundaries(ax=ax[0], data=data, GSBS=gs)
 ...
 #               scalednoise = BOLDnoise * np.repeat(np.expand_dims(stdevs * nperc, 0), ntime, axis=0)
            ...
            if rep == 0:
                plot_time_correlation_boundaries(ax=ax[idx_nperc+1], data=newdata, GSBS=GSBS_states)
                ...
                ax[idx_nperc + 1].set_xlabel('')  # Remove x-label
                ax[idx_nperc + 1].set_ylabel('')  # Remove y-label
                # Remove spines (black boxes)
                for spine in ax[idx_nperc + 1].spines.values():
                    spine.set_visible(False)
...
def simulate_time_variability(group, SL, loaddir, savedir):
    ...
    for offset in range(0,3):
        for rep in range(0, 3):
            ...
            if rep==2:
                plot_time_correlation_boundaries(ax=ax[offset], data=newdata, GSBS=GSBS_states)
    ...
    # Remove ticks, labels, and spines for all subplots
    for subplot in ax:
        subplot.set_xticks([])  # Remove x-ticks
        subplot.set_yticks([])  # Remove y-ticks
        subplot.set_xlabel('')  # Remove x-label
        subplot.set_ylabel('')  # Remove y-label
        # Remove spines (black boxes)
