...
sim_output_offset = Parallel(n_jobs=3)(delayed(simulate_time_variability)(group, SL, loaddir, savedir) for SL in SLs)
...
for iSL, SL in enumerate(SLs):
    ...
    plt.savefig(savedir + name + 'nstates.png')

#plot results offset simulation
results = np.squeeze(np.array(sim_output_offset))
plt.figure()
plt.rcParams['font.size']=18
